...
p1 <- logging %>%
  ...
  geom_line() + 
  scale_y_reverse(breaks = c(0, 0.5, 1, 1.5, 2, 2.273), labels = c("0","0.5","1.0","1.5","2.0","2.273")) + 
  scale_x_continuous(position = "top") +
  labs(x = "Temperature (°C)", y = "Depth (km)") +
  theme_tidy(ratio = 2.5) +
  theme(
...
```{r plot lithology}
i <- list(start = 0.6, end = 1.5, depth = c(.176,.953,1.268,1.270,1.311,1.448,1.537,
...
p2 <- lithology %>%
  ...
  geom_segment(data = i, aes(x = start, xend = end, y = depth, yend = depth), inherit.aes = F) +
  theme(
    panel.border = element_blank(),
