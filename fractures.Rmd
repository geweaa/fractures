...
dfluid <- read_tsv("data/mudpit_counts.tsv", col_types = cols(.default = col_double(), coreid = col_character())
                   )
...
dfluid <- read_tsv("data/samples_log.tsv", 
                   col_types = cols(.default = col_double(), 
                                    coreid = col_character(), beads_core = col_character())
                   ) %>% select(coreid, quenched = 4) %>%
  mutate(coreid = paste0("C", coreid)) %>%
  distinct() %>% na.omit() %>%
  inner_join(dfluid, by = "coreid")
...
```{r export bar plot}
ggsave("figures/cosc_fractures.pdf", width = 16, height = 10, units = "cm")
```
...
```
```{r}
ggsave("logging_nolgend.pdf", width = 8, height = 10, units = "cm")
```
